...
class HistogramDense(Agent):
    ...
    def init_neural_network(self):
        ...
        embed_dim = 32
        maxlen = 1
        embedding_layer = TokenAndPositionEmbedding( vocab_size, embed_dim)
        input_model = tf.keras.Input(shape=(1),dtype=tf.string)
        self.vectorize_layer = tf.keras.layers.TextVectorization(
            max_tokens=vocab_size,
            output_mode='int',
            output_sequence_length=maxlen)
        model_mid = self.vectorize_layer(input_model)
        model_mid = embedding_layer(model_mid)
        model_mid = tf.keras.layers.Dense(1)(model_mid)
        model_mid = tf.keras.layers.Dense(2,activation='relu')(model_mid)
    ...
    def prepare_data(self, data, in_train=False):
        ...
        normalized_data_input = np.asarray(normalized_data_input)
