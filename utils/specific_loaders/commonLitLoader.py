...
class DataUnit(object):
    ...
    def __init__(self, type_val, shape, data, name='', load_name=None, is_id=False, break_seq=False, break_size=100,
                 is_file_name=False):
        self.is_id = is_id
...
def worker_load_image_data_from_dir_tree_csv(args):
    ...
    for element in list( data_schema_input.keys()):
        ...
        if os.path.exists(dir_path + element+'.csv'):
            df = pd.read_csv(dir_path + element+'.csv', low_memory=False)
