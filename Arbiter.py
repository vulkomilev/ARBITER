...
class Arbiter(object):
    ...
    def __init__(self, img_height, img_width, class_num,tagrte_type):
        self.img_height = img_height
        self.img_width = img_width
        self.class_num = class_num
        self.tagrte_type = tagrte_type
        self.init_agents(img_height, img_width, self.class_num,tagrte_type)
    ...
    def init_neural_network(self):
        ...
        for element in dir(self):
            if 'agent_' in element:
                agent_size += 1
        if self.tagrte_type == "Regression":
            class_num = 100
        else:
            class_num =  self.class_num
        self.arbiter_neural_network_input = tf.keras.Input((agent_size * class_num))
        layer_size = agent_size
        self.arbiter_neural_network = tf.keras.layers.Dense(int(layer_size) * class_num)( \
    ...
    def init_agents(self, img_height, img_width, class_num,tagrte_type):
        self.agent_local_resnet50 = MyResNet50(img_height, img_width, class_num,tagrte_type)
